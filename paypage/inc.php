...
function wxpay_jspay($channel,$trade_no,$money,$name,$openid){
	...
	$wxinfo = \lib\Channel::getWeixin($channel['appwxmp']);
	if(!$wxinfo)showerrorjson('支付通道绑定的微信公众号不存在');
	...
	if($order["result_code"]=='SUCCESS'){
		...
		showerrorjson('微信支付下单失败！['.$order["err_code"].'] '.$order["err_code_des"]);
	}else{
		showerrorjson('微信支付下单失败！['.$order["return_code"].'] '.$order["return_msg"]);
	}
}
