...
if($conf['blockips']){
	...
	if(in_array($clientip, $blockips))exit('{"code":-1,"msg":"系统异常无法完成付款"}');
}
if($payer && $conf['blockusers']){
	$blockusers = explode('|',$conf['blockusers']);
	if(in_array($payer, $blockusers))exit('{"code":-1,"msg":"系统异常无法完成付款"}');
}
